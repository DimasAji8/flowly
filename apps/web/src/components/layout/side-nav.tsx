...
export function SideNav() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 rounded-xl px-3 py-3 border border-[var(--color-border-subtle)] bg-[var(--color-card)]">
        {avatarSrc ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={avatarSrc} alt={user?.name ?? "avatar"} width={36} height={36} className="size-9 rounded-full object-cover shrink-0" />
        ) : (
          <div className="grid size-9 shrink-0 place-items-center rounded-full bg-[var(--color-accent-soft)] text-sm font-semibold text-[var(--color-accent)] select-none">
            {user?.name ? user.name.charAt(0).toUpperCase() : "?"}
          </div>
        )}
        <div className="flex flex-col min-w-0">
          <span className="truncate text-sm font-semibold text-[var(--color-text-primary)]">{user?.name ?? "..."}</span>
          <span className="truncate text-xs text-[var(--color-text-muted)]">{user?.email ?? ""}</span>
        </div>
      </div>
    </aside>
  );
