...
class CategoryRef {
  @ApiProperty({
    example: 'cmpgcat000001s01abcdefgh',
    description: 'ID kategori',
  })
  id!: string;
  @ApiProperty({ example: 'Makanan', description: 'Nama kategori' })
  name!: string;
  @ApiProperty({ example: '#15803D', description: 'Warna kategori (hex)' })
  color!: string;
}

class WalletRef {
  @ApiProperty({ example: 'cmpgw000001s01abcdefgh', description: 'ID wallet' })
  id!: string;
...
export class TransactionResponse {
  @ApiProperty({
    example: 'cmptrx000001s01abcdefgh',
    description: 'ID transaksi',
  })
  id!: string;
  @ApiProperty({ enum: TransactionType, description: 'income atau expense' })
  type!: TransactionType;
  @ApiProperty({ example: '25000.00', description: 'Jumlah (string decimal)' })
  amount!: string;
  @ApiProperty({
    example: 'cmpgcat000001s01abcdefgh',
    description: 'ID kategori',
  ...
  walletId!: string;
  @ApiProperty({
    example: 'cmpus000001s01abcdefgh',
    description: 'ID user pembuat',
  })
  userId!: string;
  @ApiProperty({
    nullable: true,
  ...
  updatedAt!: string;
  @ApiProperty({ type: CategoryRef, description: 'Data kategori' })
  category!: CategoryRef;
  @ApiProperty({ type: WalletRef, description: 'Data wallet' })
  wallet!: WalletRef;
}
